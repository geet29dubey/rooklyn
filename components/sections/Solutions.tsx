  ...
  return (
    <section id="solutions" className="py-section-mobile sm:py-section-tablet md:py-section landscape-phone:py-10">
      <div className="site-container">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:gap-6 xl:gap-5">
...
function SolutionCard({
  ...
  return (
    <div className="relative h-full rounded-[20px] border border-hairline bg-card p-7 sm:p-8 xl:p-7">
      <div
        aria-hidden
      ...
      <div className="relative">
        <div
          className={cn(
            "mb-5 flex h-11 w-11 xl:mb-4 xl:h-10 xl:w-10 items-center justify-center rounded-xl",
            accent.bg,
            accent.text
          )}
        >
        ...
        <h3>{t(`${base}.name`)}</h3>
        <p className="eyebrow mt-3 !text-[15px] xl:!text-[13px] tracking-[0.16em] text-text-3">
          {t("solvesLabel")}: {t(`${base}.solves`)}
        </p>
        <p className="mt-4 text-[19px] leading-[1.6] text-text-3 sm:text-[20px] lg:text-[21px] xl:mt-3 xl:text-[17.5px] xl:leading-[1.55]">{t(`${base}.description`)}</p>
        <ul className="mt-5 flex flex-col gap-2.5 xl:mt-4 xl:gap-2">
          {[1, 2, 3].map((n) => (
            <li key={n} className="flex items-start gap-2.5 text-[18px] text-text-2 sm:text-[19px] lg:text-[20px] xl:text-[16.5px] xl:leading-[1.5]">
...
function CustomCard({ t }: { t: ReturnType<typeof useTranslations> }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[20px] border border-hairline bg-card p-8 sm:p-12 xl:p-9">
      ...
      <div className="relative grid gap-8 xl:gap-6 lg:grid-cols-2 lg:items-center">
        <div>
          <div className="mb-5 flex h-11 w-11 xl:mb-4 xl:h-10 xl:w-10 items-center justify-center rounded-xl bg-lagoon/12 text-lagoon">
            <Wand2 className="h-5 w-5" strokeWidth={1.5} />
          </div>
          <h3>{t(`${base}.name`)}</h3>
          <p className="mt-4 max-w-md text-[19px] leading-[1.6] text-text-3 sm:text-[20px] lg:text-[21px] xl:mt-3 xl:text-[17.5px] xl:leading-[1.55]">
            {t(`${base}.description`)}
          </p>
          <AnchorButton
            href="#contact"
        ...
        <ul className="flex flex-col gap-3 xl:gap-2.5">
          {[1, 2, 3].map((n) => (
            <li
              key={n}
              className="flex items-start gap-3 rounded-2xl border border-porcelain/[0.1] bg-white/[0.03] p-4 xl:p-3 text-[18px] text-text-2 sm:text-[19px] lg:text-[20px] xl:text-[16.5px] xl:leading-[1.5]"
            >
              <Check className="mt-0.5 h-4 w-4 shrink-0 text-champagne" strokeWidth={1.5} />
              {t(`${base}.feature${n}`)}
            </li>
